...
const EditStudent = () => {
    ...
    const fetchStudent = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${config.env.baseUrl}/users/students/${id}`,
          {
            headers: {
  ...
  const form = useForm<
    zod.infer<typeof updateStudentSchema>
  >({
  ...
  useEffect(() => {
    if (student) {
      form.reset({
        ...
        aptitudesAndStrengths:
          student.aptitudesAndStrengths || '',
        shortTermGoals:
          student.shortTermGoals || '',
        longTermObjectives:
          student.longTermObjectives || '',
        workshopsAttended:
          student.workshopsAttended || '',
        resourcesUtilized:
          student.resourcesUtilized || '',
        skillsDevelopment:
          student.skillsDevelopment || '',
        behavioralChanges:
          student.behavioralChanges || '',
        feedbackAndSelfReflection:
          student.feedbackAndSelfReflection || '',
        obstaclesEncountered:
          student.obstaclesEncountered || '',
        interventionsProvided:
          student.interventionsProvided || '',
        gender: student.gender || undefined,
        accountType:
          student.accountType || undefined,
      });
    }
  }, [student, form]);
  ...
  const onSubmit = async (
    values: zod.infer<typeof updateStudentSchema>
  ) => {
